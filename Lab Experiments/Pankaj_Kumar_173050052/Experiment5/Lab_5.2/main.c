...
* Theme: Using joystick control a object on the GLCD 
...
* Logic: iteratively calls display_image function to display a moving square box image
...
SysCtlClockSet(SYSCTL_SYSDIV_5 | SYSCTL_USE_PLL | SYSCTL_OSC_MAIN | SYSCTL_XTAL_16MHZ);
  ...
  while(1){
    ...
    // moving towards right until reach to end

    if((h_avg>=0) && (h_avg<20) && l<=pg*128-12){
    	    one[l]=0x00;
    	    one[l+1]=0x00;
    	    one[l+2]=0x00;
    	    one[l+3]=0x00;
    ...
    // moving towards left until reach to end
    
    else if(h_avg>300 && l>(pg-1)*128){
    	 one[l-1]=0xFF;
    	 one[l-2]=0xFF;
    	 one[l-3]=0xFF;
    	 one[l-4]=0xFF;
    ...
    // moving towards down until reach to end
    
    else if(v_avg>=0 && v_avg<20 && pg<8){
    	pg++;
    	one[l]=0x00;
    	    	 one[l+1]=0x00;
    	    	 one[l+2]=0x00;
    ...
    // moving towards up until reach to end
    
    else if(v_avg>400 && pg>1){
        	pg--;
        	one[l]=0x00;
        	    	 one[l+1]=0x00;
        	    	 one[l+2]=0x00;
